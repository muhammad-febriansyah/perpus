...
$sqldetil = $this->db->query("select peminjaman.*,siswa.nis,siswa.kelas from peminjaman inner join siswa on siswa.nama = peminjaman.siswa where peminjaman.tglpinjam='$pinjam' and peminjaman.status='Pinjam'");
//body

$pdf->SetFont('Times', 'B', 14);
$pdf->Cell(0, 5, 'Laporan Data Peminjaman Buku', 0, 1, 'C');
$pdf->Cell(30, 8, '', 0, 1);
...
foreach ($sqldetil->result() as $d) {
    ...
    $pdf->Cell(25, 6, $d->kelas, 1, 0);
    $pdf->Cell(30, 6, $d->buku, 1, 0);
    $pdf->Cell(30, 6, $d->kondisipinjam, 1, 0);
    $pdf->Cell(32, 6, format_indo(date("Y-m-d"),$d->tglpinjam), 1, 1);
}

$pdf->SetFont('Times', '', 10);

$pdf->Output('datapeminjamanbuku.pdf', 'I');
